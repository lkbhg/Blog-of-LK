...
以下是CVAT官方采用的基础环境需求（实际上对于大多数系统来说，只需要直接安装`docker`相关组件即可，无需注册GPG key）：
```bash
...
为保证docker的socket能够被用户正确访问，可以选择加入docker用户组，或者每次使用sudo来调用docker相关命令
```bash
...
1. 在CVAT文件夹中使用：
```bash
echo "CVAT_HOST=10.17.157.105" > .env
```
